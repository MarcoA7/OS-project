...
int main(int argc, char const *argv[])
{
    int shm_fd;
	void *ptr;
    int shared_seg_size = (1 * sizeof(struct mat *));
	struct mat * myMatrix;
    ...
    myMatrix = (struct mat *)mmap(NULL, shared_seg_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (myMatrix == NULL)
    {
        perror("In mmap()");
        exit(1);
    }
	myMatrix = malloc(sizeof(struct mat * ));
	myMatrix->rows = 9;
	myMatrix->cols = 9;
	myMatrix->data = (int**)malloc(sizeof(int*) * myMatrix->rows);
	for(int i = 0; i < myMatrix->rows; i++) 
		myMatrix->data[i] = (int*)malloc(sizeof(int) * myMatrix->cols);
	myMatrix->data[0][0] = 9;
	int i = 0;
	int j = 0;
	printf("%d\n", myMatrix->data[i][j]);
